...
```{r fig1, fig.height = 5, fig.width = 10, fig.align = "center", warning = FALSE}
...
The exemplary dataset is a whole transcriptome dataset, which is why the `exp_type` parameter is set to `WTA`. The alternative would be to state `Amplicon`. The `symbols` parameter is for retrieving NCBI gene names for the ensemble gene names present in the raw data with the help of the `r Biocpkg("biomaRt")` package. This parameter is not abundant to state at function execution as the standard value is `biomart`. For offline execution, `orgdb` is suggested, which uses the `r Biocpkg("org.Hs.eg.db")` package.
...
## Filtering low abundance features
